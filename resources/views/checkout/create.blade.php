...
<main class="max-w-3xl mx-auto px-6 py-20">
    ...
    {{-- Flash Success --}}
    @if(session('success'))
    <div class="mb-6 p-4 bg-green-100 text-green-700 rounded-xl font-bold flex items-center gap-3">
    ...
    <div class="grid grid-cols-1 gap-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            @auth
                <div class="flex flex-col md:flex-row items-center gap-4 p-4 bg-indigo-50 border border-indigo-100 rounded-2xl mb-8">
                    @if(auth()->user()->google_avatar)
                        <img src="{{ auth()->user()->google_avatar }}" alt="Avatar" class="w-12 h-12 rounded-full border-2 border-indigo-200">
                    @else
                    ...
                    @endif
                    <div>
                        <p class="text-xs text-slate-500 font-bold uppercase tracking-wider">Telah Login via Google SSO</p>
                        <p class="font-extrabold text-slate-800">{{ auth()->user()->name }} ({{ auth()->user()->email }})</p>
                    </div>
                    <button type="button" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="md:ml-auto text-xs font-bold text-red-500 hover:text-red-700 hover:underline">
            ...
            @else
                <div class="mb-8 p-6 bg-slate-50 border border-slate-200 rounded-3xl text-center">
                    <p class="text-sm font-bold text-slate-600 mb-4 uppercase tracking-wider">
                        ⚡ Pesan Tiket Instan Tanpa Kerumitan Form
                    </p>
                    <a href="{{ route('auth.google', ['event_id' => $event->id]) }}" 
                       class="inline-flex items-center justify-center gap-3 px-6 py-4 bg-white border border-slate-300 hover:border-indigo-500 rounded-2xl font-bold text-slate-700 hover:text-indigo-600 shadow-sm hover:shadow transition-all duration-200 w-full md:w-auto">
            ...
            <h3 class="text-xl font-bold mb-6 italic text-indigo-600 underline underline-offset-8">
                📦 Data Pemesan {{ auth()->check() ? '(Terautentikasi)' : '(Tanpa Login)' }}
            </h3>
